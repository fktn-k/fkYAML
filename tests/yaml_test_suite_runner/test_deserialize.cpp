...
TEST_CASE("yaml_test_suite_2EBW") {
    ...
    SUBCASE("YAML") {
        ...
        REQUIRE(n.at("a!\"#$%&\'()*+,-./09:;<=>?@AZ[\\]^_`az{|}~").as_str() == "safe");
...
TEST_CASE("yaml_test_suite_6HB6") {
    ...
    SUBCASE("YAML") {
        ...
        REQUIRE(n.at("Not indented").at("By one space").as_str() == "By four\n  spaces\n");
        ...
        REQUIRE(n.at("Not indented").at("Flow style").at(1).as_str() == "Also by two");

        REQUIRE(n.at("Not indented").at("Flow style").at(2).is_string());
        REQUIRE(n.at("Not indented").at("Flow style").at(2).as_str() == "Still by two");
    }
...
TEST_CASE("yaml_test_suite_7BMT") {
    ...
    SUBCASE("YAML") {
        ...
        auto top2_it = n.at("top2").begin();

        auto& top2_key = top2_it.key();
        REQUIRE(top2_key.is_string());
        REQUIRE(top2_key.as_str() == "key2");

        auto& top2_val = top2_it.value();
        ...
        auto top5_it = n.at("top5").begin();

        auto& top5_key = top5_it.key();
        REQUIRE(top5_key.is_string());
        REQUIRE(top5_key.as_str() == "key5");

        auto& top5_val = top5_it.value();
